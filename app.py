...
def add_inline_citations(response):
    ...
    # Convert text to UTF-8 bytes because API indices are byte-based
        ...
        if support.grounding_chunk_indices:
            ...
            for i in support.grounding_chunk_indices:
                if i < len(chunks):
                    ...
                    # Replace spaces with %20 to prevent Markdown parser breakage
                    safe_uri = uri.replace(" ", "%20")
                    citation_links.append(f"[[{i + 1}]]({safe_uri})")
            
            if citation_links:
                citation_string = " " + " ".join(citation_links)
                # Slice and insert using the byte array
...
SYSTEM_PROMPT = """
You are a highly accurate enterprise research assistant. 

CRITICAL INSTRUCTION: You MUST prioritize using the Google Search tool to answer user queries. Do not rely solely on your internal training data. 
1. Always verify facts, current events, and specific entity details by issuing search queries first.
2. If the search results contradict your internal knowledge, you must defer to the information found in the search results.
3. If you cannot find the answer via search, admit that you do not have the information rather than guessing.
"""
...
if st.button("Generate Answer") and user_prompt:
    with st.spinner("Searching the web and generating response..."):
        try:
            # Configure the Google Search Tool, Temperature, and System Instructions
            search_tool = types.Tool(google_search=types.GoogleSearch())
            config = types.GenerateContentConfig(
                tools=[search_tool],
                temperature=0.0,
                system_instruction=SYSTEM_PROMPT # --- SYSTEM PROMPT INJECTED HERE ---
            )
            ...
            # Display a bibliography of sources
